...
import { fetchEmployeeById, editEmployee, checkEmailExists } from "reduxHilo/actions/employeeAction";
...
const EditEmployeeForm = ({ isOpen, onClose, fetchEmployees, employeeId }) => {
  ...
  const validate = async () => {
    ...
    if (!formData.email) {
      ...
      try {
        ...
        if (emailExists && formData.email !== employee.email) {
    ...
    else if (!/^\d{10}$/.test(formData.phone)) validationErrors.phone = 'Phone number must be 10 digits';
    ...
    if (!formData.birthdate) validationErrors.birthdate = 'Birthdate is required';
      ...
      if (age < 18) validationErrors.birthdate = 'Employee must be at least 18 years old';
    }
  ...
  const handleSubmit = async (e) => {
    ...
    if (Object.keys(validationErrors).length > 0) {
      ...
      try {
        await dispatch(editEmployee(employeeId, formData));
        setAlertMessage("Employee edited successfully");
  ...
  const handleCloseAlert = () => {
    setShowAlert(false);
    onClose(); // Chỉ đóng form sau khi Alert được đóng
  };
