...
import { addCustomer, fetchCustomers, checkEmailExists } from "reduxHilo/actions/customerAction"; // Điều chỉnh các action
...
const AddCustomerForm = ({ isOpen, onClose }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            await dispatch(addCustomer(formData)); // Sử dụng action thêm khách hàng
            setAlertMessage("Customer added successfully");
            setAlertType("success");
            setShowAlert(true);
            onClose();
            dispatch(fetchCustomers()); // Làm mới danh sách khách hàng
        } catch (error) {
