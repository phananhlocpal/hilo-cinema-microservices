...
import { fetchCustomerById, editCustomer, checkEmailExists } from "reduxHilo/actions/customerAction"; // Điều chỉnh các action
import ModalAlert from "components/alert/modalAlert";

const EditCustomerForm = ({ isOpen, onClose, fetchCustomers, customerId }) => {
  ...
  const validate = async () => {
    ...
    if (!formData.email) {
      ...
      try {
        ...
        if (emailExists && formData.email !== customer.email) {
    ...
    if (!formData.birthdate) {
      ...
      if (age < 18) {
        validationErrors.birthdate = 'Customer must be at least 18 years old';
      }
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await dispatch(editCustomer(customerId, formData)); // Sử dụng action chỉnh sửa khách hàng
